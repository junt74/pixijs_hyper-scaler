...
class OBJECT_OT_pixijs_hyper_scaler_prepare_sprite_exports(Operator):
    bl_idname = "object.pixijs_hyper_scaler_prepare_sprite_exports"
    bl_label = "Prepare Sprite Exports"
    bl_description = (
        "Apply modifiers, separate by loose parts, and set origin to geometry "
        "for selected mesh objects"
    )
    ...
    def execute(self, context):
        ...
        try:
            for source_obj in selected_meshes:
                bpy.ops.object.select_all(action="DESELECT")
                source_obj.select_set(True)
                context.view_layer.objects.active = source_obj

                modifier_names = [modifier.name for modifier in source_obj.modifiers]
                for modifier_name in modifier_names:
                    try:
                        ...
                        self.report(
                            {"ERROR"},
                            f'Failed to apply modifier "{modifier_name}" on "{source_obj.name}": {exc}',
                        )
                        return {"CANCELLED"}
                ...
                split_objects = [obj for obj in context.selected_objects if obj.type == "MESH"]
                if not split_objects:
                    split_objects = [source_obj]

                bpy.ops.object.origin_set(type="ORIGIN_GEOMETRY", center="MEDIAN")
                resulting_objects.extend(split_objects)
            ...
            if resulting_objects:
                context.view_layer.objects.active = resulting_objects[0]

            self.report(
                {"INFO"},
                f"Prepared {len(resulting_objects)} mesh object(s) for sprite export",
            )
            return {"FINISHED"}
        finally:
            if not resulting_objects:
                bpy.ops.object.select_all(action="DESELECT")
...
class VIEW3D_PT_pixijs_hyper_scaler_stage_export(Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.label(text="Sprite Prep:")
        layout.operator(
            OBJECT_OT_pixijs_hyper_scaler_prepare_sprite_exports.bl_idname,
            icon="MOD_ARRAY",
        )
        layout.separator()
...
CLASSES = (
    OBJECT_OT_pixijs_hyper_scaler_prepare_sprite_exports,
